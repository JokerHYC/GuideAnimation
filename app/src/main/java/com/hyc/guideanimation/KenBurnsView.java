...
public class KenBurnsView extends FrameLayout {
    ...
    private void forceSwapImage() {
        ...
        animatorSet.playTogether(ObjectAnimator.ofFloat(inactiveImageView, PROPERTY_ALPHA, 1.0f, 0.0f), ObjectAnimator.ofFloat(activeImageView, PROPERTY_ALPHA, 0.0f, 1.0f));
        animatorSet.start();
    }
    ...
    public void loadResourceIDs(List<Integer> resourceIDs) {
        ...
        Log.e(TAG, "loadResourceIDs: " +( mRootLayout == null ? "null " : "not null"));
        if (mRootLayout != null) {
            initImageViews(mRootLayout);
        }
    }
...
//        }
    ...
    private void initRunnable() {
        ...
        mSwapImageRunnable = new Runnable() {
            @Override
            public void run() {
                autoSwapImage();
        ...
        mForceSwapImageRunnable = new Runnable() {
            @Override
            public void run() {
                forceSwapImage();
                mHandler.postDelayed(mSwapImageRunnable, mSwapMs - mFadeInOutMs * 2);
    ...
    private void loadImage(final int position, final int imageIndex) {
        switch (mLoadType) {
            case String:
                Glide.with(mContext).load(mStrings.get(position)).dontAnimate().into(mImageViews[imageIndex]);
                break;
            case ResourceID:
                Glide.with(mContext).load(mResourceIDs.get(position)).dontAnimate().into(mImageViews[imageIndex]);
//                mImageViews[imageIndex].setImageResource(mResourceIDs.get(position));
            ...
            case MIXING:
                ...
                if (object.getClass() == String.class) {
                    ...
                    Glide.with(mContext).load(string).dontAnimate().into(mImageViews[imageIndex]);
                } else if (object.getClass() == Integer.class) {
                    Integer integer = (Integer) object;
//                    mImageViews[imageIndex].setImageResource(integer);
                    Glide.with(mContext).load(integer).dontAnimate().into(mImageViews[imageIndex]);
                }
                break;
        }
    }
